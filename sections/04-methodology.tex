...
Let $p(c\mid x)$ be the model's posterior over the $C{=}200$ classes, obtained by
applying a softmax with temperature $T$ (tuned on a held-out validation set) to
the species logits. Instead of the arg\,max, CRM~\cite{karthik2021nocost}
...
close to the likely candidates. CRM is fully post-hoc and requires no
retraining.
